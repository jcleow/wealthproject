...
export function calculateVES(
  ...
  for (const band of bands) {
    ...
    if (co2 <= bandEntry.maxCo2) {
      return bandEntry.amount
    }
  }
  return 0
}
...
export function calculateRegistrationCost(inputs: VehicleInputs) {
  ...
  const totalRegistrationCost =
    inputs.omv + exciseDuty + gst + arf + inputs.coePrice + REGISTRATION_FEE + vesAmount + eeaiRebate
...
export function calculateRoadTax(
  ...
  if (isEv && !isMotorcycle) {
    sixMonthly += EV_AFC_SIX_MONTHLY
  }
...
export function calculateFinancing(inputs: VehicleInputs, totalRegistrationCost: number) {
  ...
  if (isMotorcycle) {
    maxLtvPercent = 1.0 // No LTV restriction for motorcycles
  } else if (inputs.omv <= LTV_LIMITS.lowOmv.threshold) {
    maxLtvPercent = LTV_LIMITS.lowOmv.maxLtv
  } else {
    maxLtvPercent = LTV_LIMITS.highOmv.maxLtv
  }
  ...
  // Exact EIR formula: 2 × n × flatRate / (n + 1)
...
function getDepreciationRateForYear(year: number, periods: DepreciationPeriod[]): number {
  ...
  return 0.05 // Fallback 5% if no period matches
}

function getPARFRebatePercent(ageAtDereg: number): number {
  for (const entry of PARF_REBATE_SCHEDULE) {
    if (ageAtDereg <= entry.maxAge) {
      return entry.rebatePercent
    }
  }
  return 0 // Beyond 10 years: no PARF
}
...
export function calculateDepreciation(
  ...
  for (let year = 1; year <= 10; year++) {
    ...
    if (isPARFEligible && ageAtDereg <= 10) {
      parfRebate = Math.min(Math.round(arf * getPARFRebatePercent(ageAtDereg)), PARF_CAP)
    }

    // COE rebate: pro-rated by remaining months
...
export function calculateTCO(
  ...
  const upfrontCosts =
    inputs.condition === 'used' ? inputs.listPrice : calculationResult.totalRegistrationCost
  ...
  // Residual value at end of ownership period
  const yearIndex = Math.min(ownershipYears, calculationResult.depreciationSchedule.length) - 1
  const residualValue =
    yearIndex >= 0 ? calculationResult.depreciationSchedule[yearIndex].scrapValue : 0

  // Interest paid over ownership period (may be less than full loan if loan is shorter)
...
export function calculateVehicle(
  ...
  const depreciationBase =
    inputs.condition === 'used' ? inputs.listPrice : registration.totalRegistrationCost
