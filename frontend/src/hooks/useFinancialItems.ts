...
/**
 * Hook for accessing financial items.
 * Lazily fetches v1 financial data only when this hook is used.
 */
export function useFinancialItems(): UseFinancialItemsReturn {
  // These queries are only triggered when this hook is used
  ...
  const getItemsForType = useMemo(() => {
    return (targetType: string): FinancialItem[] => {
      switch (targetType) {
        case 'income':
          return incomes.map(inc => ({ id: inc.parentId ?? inc.id, name: inc.name, amount: inc.amount, frequency: inc.frequency }))
        case 'expense':
          return expenses.map(exp => ({ id: exp.parentId ?? exp.id, name: exp.name, amount: exp.amount, frequency: exp.frequency }))
        case 'asset':
