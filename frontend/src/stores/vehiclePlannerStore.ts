    ...
    persist(
      (set) => ({
        ...
        setOwnershipYears: (years) =>
          set({ ownershipYears: Math.max(1, Math.min(10, years)) }),
        ...
        updateScenarioInputs: (id, updates) =>
          set((state) => ({
            scenarios: state.scenarios.map((s) =>
              s.id === id
                ? { ...s, inputs: { ...s.inputs, ...updates }, updatedAt: Date.now() }
                : s
            ),
          })),
        ...
        updateScenarioRecurringCosts: (id, updates) =>
          set((state) => ({
            scenarios: state.scenarios.map((s) =>
              s.id === id
                ? { ...s, recurringCosts: { ...s.recurringCosts, ...updates }, updatedAt: Date.now() }
                : s
            ),
          })),
        ...
        toggleIncluded: (id) =>
          set((state) => ({
            scenarios: state.scenarios.map((s) =>
              s.id === id
                ? { ...s, isIncluded: !s.isIncluded, updatedAt: Date.now() }
                : s
            ),
          })),
      }),
...
export const useActiveVehicleScenario = () =>
  useVehiclePlannerStore(
    (s) => s.scenarios.find((scenario) => scenario.id === s.activeScenarioId) ?? null
  )
