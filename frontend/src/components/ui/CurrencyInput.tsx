...
export function CurrencyInput({
  ...
  return (
    <div className={cn("relative", className)}>
      {!isPercentage && (
        <span className={cn("absolute left-3 top-1/2 -translate-y-1/2 text-slate-500", textSize)}>$</span>
      )}
      <input
        type="text"
        inputMode="numeric"
        ...
        className={cn(
          "w-full py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] text-white font-mono tabular-nums focus:outline-none focus:border-white/20 placeholder:text-slate-600",
          textSize,
          isPercentage ? "pl-3 pr-6" : "pl-7 pr-3"
        )}
      />
      {isPercentage && (
        <span className={cn("absolute right-3 top-1/2 -translate-y-1/2 text-slate-500", textSize)}>%</span>
      )}
    </div>
  )
}
