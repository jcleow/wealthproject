...
describe('calculateAllocationEndDate', () => {
  describe('basic functionality', () => {
    it('should return last day of previous month', () => {
      ...
      // Use UTC methods since function stores dates in UTC
      expect(date.getUTCFullYear()).toBe(2027)
      expect(date.getUTCMonth()).toBe(2) // March (0-indexed in JS Date)
  ...
  describe('regression test for bug #4051', () => {
    it('should NOT produce year 4051 in end date', () => {
      ...
      const date = new Date(result)
      expect(date.getUTCFullYear()).toBe(2026)
      expect(date.getUTCFullYear()).not.toBe(4051)
    })
    ...
    it('should produce correct date for "year 2 April" scenario', () => {
      ...
      expect(dateActual.getUTCFullYear()).toBe(2027)
      expect(dateActual.getUTCMonth()).toBe(2) // March
      ...
      expect(dateOffset.getUTCFullYear()).toBe(2027)
      expect(dateOffset.getUTCMonth()).toBe(2) // March
    })
  })
})
