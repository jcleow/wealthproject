...
ADD COLUMN IF NOT EXISTS cpf_wage_type TEXT,
...
-- Add constraint for valid CPF wage types (OW = Ordinary Wages, AW = Additional Wages)
ALTER TABLE finance_incomes
ADD CONSTRAINT finance_incomes_cpf_wage_type_check CHECK (
    cpf_wage_type IS NULL OR cpf_wage_type IN ('ow', 'aw')
);

COMMENT ON COLUMN finance_incomes.income_type IS 'Type of income: salary, bonus, commission, rental, dividend, freelance, other';
COMMENT ON COLUMN finance_incomes.cpf_wage_type IS 'CPF wage classification: ow (Ordinary Wages - monthly), aw (Additional Wages - bonus/irregular), null (non-employment)';
