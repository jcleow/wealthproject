...
// NOTE: Income allocation methods have been removed - use fund_flow_rules API instead
// The fund_flow_rules table with rule_type='allocation' replaces the legacy income_allocations table.
// See ListFundFlowRules, CreateFundFlowRule, etc. for the new API.
...
func (s *Store) ResetAllUserData(ctx context.Context, userID string) (int64, error) {
	...
	// 1. Delete fund_flow_rules first (depends on incomes, investments, cash_accounts, liabilities, properties)
	tag, err := tx.Exec(ctx, `DELETE FROM fund_flow_rules WHERE user_id = $1`, userID)
	if err != nil {
		return 0, fmt.Errorf("failed to delete fund flow rules: %w", err)
	}
