...
func (s *Service) ProjectCPFBalanceProjection(ctx context.Context, userID, cpfAccountID string, retirementAge, payoutStartAge int, assumptions *projector.ProjectionAssumptions) (*BalanceProjectionResult, error) {
	...
	// Get all incomes for this person
	incomeResult, err := s.store.ListIncomes(ctx, repo.ListQuery{
		UserID: userID,
	})
	if err != nil {
	...
	// Filter incomes to only those belonging to this person
	var personIncomes []projector.IncomeStream
	for _, inc := range incomeResult.Data {
		if inc.PersonID != cpfAccount.PersonID {
			continue
		}
		incAmount := inc.Amount // Create local copy
		stream := projector.IncomeStream{
			MonthlyAmount: &incAmount,
			WageType:      "ow", // Default to ordinary wages
			StartDate:     inc.StartDate,
			EndDate:       inc.EndDate,
		}
		personIncomes = append(personIncomes, stream)
	}
	...
	// Map gender for CPF LIFE calculation
	var gender payout.Gender
	if person.Gender == "female" {
		gender = payout.GenderFemale
	} else {
	...
	// Map age 55 balances
	if projection.Age55Balances != nil {
		result.Age55Balances = &BalanceSnapshot{
			OA:       projection.Age55Balances.OA,
			SA:       projection.Age55Balances.SA,
			MA:       projection.Age55Balances.MA,
			RA:       projection.Age55Balances.RA,
			AsOfDate: projection.Age55Balances.AsOfDate,
		}
	}

	// Map age 65 balances and calculate CPF LIFE estimates
	if projection.Age65Balances != nil {
		result.Age65Balances = &BalanceSnapshot{
			OA:       projection.Age65Balances.OA,
			SA:       projection.Age65Balances.SA,
			MA:       projection.Age65Balances.MA,
			RA:       projection.Age65Balances.RA,
			AsOfDate: projection.Age65Balances.AsOfDate,
		}

		// Calculate CPF LIFE estimates using projected RA at 65
		if projection.Age65Balances.RA != nil && !projection.Age65Balances.RA.IsZero() {
			estimates, err := payout.CalculateAllPlans(
				person.DateOfBirth.Year(),
				gender,
				projection.Age65Balances.RA,
				payoutStartAge,
			)
			if err == nil {
				result.CPFLifeEstimates = &CPFLifeEstimateResult{
					RABalanceAt65:  projection.Age65Balances.RA,
					PayoutStartAge: payoutStartAge,
					BirthYear:      person.DateOfBirth.Year(),
					Gender:         string(gender),
					Standard: PlanEstimate{
